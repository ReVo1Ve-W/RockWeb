...
// 全局错误处理中间件：放在所有路由的最后。
// multer 遇到"文件太大"或"文件类型不对"时，会抛出一个错误交给 Express，
// 如果没有这个中间件接住，Express 默认会返回一个 HTML 错误页面，
// 前端 axios 解析不了 HTML，会显示成一堆乱码。这里统一把错误转成 JSON 格式，
// 前端就能正常读取 err.response.data.message 显示给用户
